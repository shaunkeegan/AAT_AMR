...
plot_multiplot.insecticide <- function(df2, Wn, p.ins, col.var, shape.var,
  ...
  p <- ggplot(plot_this, aes(x = treat_prop, y = y*0.0001, colour = prop.insecticide, group = interaction(W_st, prop.insecticide)) )  + my_theme() +
    geom_point(aes(shape = W_st), size = 2) + geom_line() +
    xlab("\n") + ylab("") +
    guides(col = guide_legend("Proportion \nInsecticide"), shape = guide_legend("Wildlife"))
  
  plots <- list()
  for (i in 1:5){
    plots[[i]] <- plot_this %>% filter(prop.insecticide == p.ins[i]) %>% 
      ggplot(aes(x = treat_prop, y = y*0.0001, group = interaction(W_st, prop.insecticide)) ) + my_theme() +
      geom_point(aes(shape = W_st), size = 2, colour = my_cols[i] ) + geom_line(colour = my_cols[i]  ) + ylim(ymin, ymax) +xlab("\n") + ylab("")+ 
