...
set1 <- function(output, birth.adj, fit.adj, K, prop_treat, prop.insecticide, NW, prop.prophylaxis){
  ...
  death.p          <- death.c
  sigma.c         <- 1 / 100
  treatment.q      <- 1*(1 + 1*runif(1)) * prop_treat * (sigma.c + death.c) / (1 - prop_treat)
  treatment.p      <- 1*(1 + 1*runif(1)) * prop_treat * (sigma.c + death.c) / (1 - prop_treat)
  sigma.st      <- sigma.c * 11 #* 250 #LM: adjusted so that R0 drops below 1 when 99% treated to reflect Hargrove
  emergence.p      <- 0#1/350
  emergence.f      <- 0#1/50
  rec.adj          <- 1
  waning           <- 1/20
